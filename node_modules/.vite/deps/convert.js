...
function s(i2, a2) {
  return "best" === i2 ? function(i3, a3, s2) {
    ...
    const u2 = r(a3, i3, c2);
    return { quantity: u2, unit: c2, toString: (e2) => `${"bigint" == typeof u2 || e2 === void 0 ? u2 : u2.toFixed(e2)} ${c2}` };
  }(this.i, this.o, a2 ?? "metric") : r(this.o, this.i, i2);
}
function m(e2, i2) {
  if (!(i2 in t)) throw new RangeError(i2 + " is not a valid unit");
  return { to: s.bind({ i: e2, o: i2 }) };
}
var o = /(-?(?:\d+)?\.?\d+)\s?(\S+)/g;
function c(e2, t2) {
  const i2 = "best" === e2;
  let a2, r2 = 0, s2 = true;
  do {
    const t3 = m(Number(this.u[1]), this.u[2]).to(i2 && !s2 ? a2 : e2);
    i2 && s2 ? (r2 += t3.quantity, a2 = t3.unit, s2 = false) : r2 += t3, this.u = o.exec(this.k);
  } while (this.u);
  return i2 ? m(r2, a2).to("best", t2) : r2;
}
function n(e2) {
  o.lastIndex = 0;
  const t2 = o.exec(e2);
  if (!t2) throw new RangeError("value did not match expression " + o.source);
  return { to: c.bind({ u: t2, k: e2 }) };
}
function l(e2) {
  const i2 = t[e2];
  if (i2) return i2[0];
...
function u(e2) {
  return "string" == typeof e2 ? n(e2).to("ms") : "" + m(e2, "ms").to("best");
}
export {
  m as convert,
  n as convertMany,
